...
- **2026-09-13 (unreleased, after v1.9.5; additive):** three changes around this helper. (1) `bbR2Fetch` treats an intentional abort (AbortController, e.g. a cancelled upload) as a breadcrumb with `aborted: true` and no error row; timeouts (`AbortSignal.timeout`) still record. (2) It uses a `Request` input's own method and accepts a null/omitted `init`. (3) The network hook now also instruments `XMLHttpRequest`, so Firebase Storage SDK and axios uploads appear as `source: 'network'` instead of being invisible. `bbR2Fetch` still uses native fetch, so it doesn't double-record. The "progress events are a different problem (XHR vs fetch)" trade-off above still holds: XHR is captured for failures, not upload progress.
