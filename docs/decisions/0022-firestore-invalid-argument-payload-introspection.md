...
- This decision is additive to ADR-0009 (the original `bbWrapWrites` ADR).
  ...
  still holds.
- **2026-09-13 (unreleased, after v1.9.5; additive):** two changes. (1) The walker now mirrors Firestore's own `isPlainObject` rule: it descends only into arrays and plain objects. `DocumentReference`, `Timestamp`, `FieldValue` sentinels etc. are leaves, and `payloadShape` shows them by class name (e.g. `DocumentReference`). Walking into a `DocumentReference` reached the Firestore instance and produced bogus paths like `author.firestore._settings.credentials`. This resolves the `arrayUnion(...)` watch item above in the conservative direction: an `undefined` inside sentinel arguments is no longer reported (Firestore's own message still names the document). (2) `bbFirestoreOp` accepts `() => promise` as well as a promise. The SDK throws `invalid-argument` synchronously, before any promise exists, so a finished promise argument could never capture it; docs now say to pass the function form for writes or use `bbWrapWrites`.
