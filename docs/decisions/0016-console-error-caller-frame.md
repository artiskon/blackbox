...
- **2026-09-13 (unreleased, after v1.9.5; additive):** `extractTopAppFrame` now returns the RAW frame (real chunk filename, `:line:col`, origin) on purpose, because `context.callerFrame` is where line numbers matter. It used to rewrite chunk hashes inside the returned frame, which mangled `callerFrame`, while line:col still leaked into the fingerprint. Fingerprinting now normalizes the frame separately (`normalizeFrameForFingerprint`, see ADR-0019), so the selection rule ("first non-framework frame") is still shared but the text differs: `groupingInputs.topFrame` is normalized, `callerFrame` is raw. Frame detection now recognizes Firefox/Safari `fn@url:line:col` frames and no longer treats the V8 `Error: <message>` line as a frame when the message contains "at ".
