...
- **2026-09-13 (unreleased, after v1.9.5; additive):** on the client `bbWrapWrites` now returns a copy of the passed object with only `addDoc` / `setDoc` / `updateDoc` / `deleteDoc` wrapped, so other keys (`getDoc`, `writeBatch`, ...) survive, matching the server passthrough and the `T` return type (they used to be dropped on the client only). Recorded messages now name the collection (see ADR-0003).
