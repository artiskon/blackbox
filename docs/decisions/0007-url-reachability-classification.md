...
- **2026-09-13 (unreleased, after v1.9.5; additive):** two network-hook changes in the same spirit ("honest I-don't-know beats confident wrong guesses").
  1. **No more `cors_blocked` on network errors.** The network hook still set `cors_blocked: true` plus `preflight_trigger_method` / `preflight_trigger_headers` / `preflight_reason` whenever Chrome said "Failed to fetch", which it says for CORS, DNS failure, refused connection, offline and ad-blockers alike. Those fields are removed. Same-origin failures (which can't be CORS) carry no classification; cross-origin failures get a no-cors HEAD probe, then `urlReachability: 'opaque_response'` + `statusHint: 'reachable_but_request_blocked_cors_likely_check_network_tab'` + `preflight_if_cors: { method, headers?, reason? }`, or `urlReachability: 'unreachable_origin'` + `statusHint: 'origin_dns_or_refused'`. The error row is recorded after the probe, usually a few ms later. The probe is capped at 2s (a hung origin, the usual cause of a timed-out request, would hang the HEAD too and the row would never land); on timeout the row gets `urlReachability: 'unknown'` + `statusHint: 'probe_timed_out_origin_slow_or_hung'`.
  2. **Opaque responses aren't failures.** Successful `mode: 'no-cors'` and `redirect: 'manual'` fetches return status 0 / `ok: false` by design and were recorded as `HTTP 0` errors. They're now breadcrumbs with `responseType: 'opaque'` / `'opaqueredirect'` and no error row.
