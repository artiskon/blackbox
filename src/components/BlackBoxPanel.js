  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderBottom: '1px solid rgba(255,255,255,0.1)', flexShrink: 0 }}>
        ...
        <span onClick={() => setIsOpen(false)} style={{ cursor: 'pointer', fontSize: '16px', color: '#999', padding: '4px 8px', marginRight: '-8px', borderRadius: '4px' }}>✕</span>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
        {tab === 'live' && (
          <div>
            {[...errors].reverse().length === 0 ? (
              <div style={{ padding: '24px 14px', textAlign: 'center', color: '#22c55e' }}>No errors captured</div>
            ) : [...errors].reverse().map((err, i) => {
              const isExpanded = expandedError === i;
              const last5 = (err.breadcrumbs || []).slice(-5);
              return (
                <div key={i}>
                  <div onClick={() => setExpandedError(isExpanded ? null : i)} style={{ padding: '8px 14px', cursor: 'pointer', borderBottom: '1px solid rgba(255,255,255,0.05)', background: isExpanded ? 'rgba(255,255,255,0.05)' : 'transparent' }}>
          ...
          <div>
            ...
            {!isConnected ? (
              ...
              <>
                ...
                {historyErrors.length === 0 && !timelineLoaded && timeline.length === 0 ? (
                  <div style={{ padding: '24px 14px', textAlign: 'center', color: '#22c55e' }}>No saved errors</div>
                ) : (
                  <>
                    {historyErrors.length > 0 && (
                      <>
                        <div style={sectionTitle}>Saved Errors ({historyErrors.length})</div>
